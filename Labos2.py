...
def listClauses():
    clauses = []
    with open("new_pizza.txt", 'r', encoding = 'utf-8') as f:
        text = f.readlines()
        for line in text:
            line = line.rstrip()
            line = line.lower()
...
def listUserCommands():
    commands = []
    with open("new_pizza_input.txt", 'r', encoding = 'utf-8') as f:
        text = f.readlines()
        for line in text:
            line = line.rstrip()
            line = line.lower()
...
def cooking(clauses, user_commands):
    ...
    for command in user_commands:
        print(command[0])
        txt = ''
        txt += "User's command: " + command[0] + " " + command[1] + '\n'
        if(command[1] == '?'):
...
clauses = listClauses()
commands = listUserCommands()

#resolution(clauses[:-1], clauses[-1])
